document.addEventListener('DOMContentLoaded', () => {
    class PlayerSelection {
        constructor() {
            ...
            this.filteredGroups = [];
            this.isReturningFromDrawing = false; // Flag to track if returning from drawing page
        ...
        async init() {
            ...
            // Always populate and show the group grid (Step 1)
            ...
            if (isReturning && storedGroupId && this.groupData[storedGroupId]) {
                ...
                // Highlight the selected group in Step 1
                const selectedCard = document.querySelector(`[data-group="${storedGroupId}"]`);
                if (selectedCard) {
                    selectedCard.classList.add('selected');
                }
                // Show Step 2 (player selection) without scrolling
        ...
        selectGroup(groupId) {
            ...
            // Show player selection step with smooth scroll
            this.showPlayerStep(group);
            this.populatePlayerGrid(group);
        }
        ...
        showPlayerStep(group) {
            ...
            if (playerStep) {
                ...
                console.log('Player step displayed, offsetTop:', playerStep.offsetTop);
            }
            if (backBtn) {
            ...
            // Scroll behavior:
            // - On group selection: Smooth scroll to Step 2
            // - On return from drawing page: Instant position at Step 2
            if (this.isReturningFromDrawing) {
                window.scrollTo({ top: playerStep.offsetTop, behavior: 'instant' });
                console.log('Instant position to Step 2, scrollY:', window.scrollY);
                this.isReturningFromDrawing = false; // Reset flag for subsequent group selections
            } else {
                console.log('Triggering smooth scroll to Step 2');
                setTimeout(() => {
                    if (playerStep) {
        ...
        proceedToDrawing() {
            ...
            localStorage.setItem('selectedPlayer', JSON.stringify({
                id: `Group ${group.groupNo} ${group.name}-${playerData.name}`,
